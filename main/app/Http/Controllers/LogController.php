...
class LogController extends Controller
{
    ...
    public function p2pLog(Request $request)
    {
        ...
        $data['transactions'] = Transaction::with('user')->when($request->trx, function ($item) use ($request) {
            ...
            $item->whereDate('created_at', $request->date);
        })->where('type_two',7)->where('user_id', auth()->id())->latest()->with('user')->paginate(Helper::pagination());

        return view(Helper::theme() . 'user.p2p.index')->with($data);
    }
    ...
    {
        $user = auth()->user();
        $data['reward'] = Transaction::where('type_two',3)->where('user_id',$user->id)->paginate(Helper::pagination());;
        return view(Helper::theme(). 'user.direct_reward')->with($data);
    }
}
