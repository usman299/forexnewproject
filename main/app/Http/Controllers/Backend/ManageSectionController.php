...
class ManageSectionController extends Controller
{
    ...
    public function overviewSection(Request $request)
    {

        $data['sections'] = Config::sections();
        $overview = 'overview';
        $data['elements'] = Content::where('theme', Helper::config()->theme)->where('type', 'iteratable')->where('name', $overview)->get();

        $elementBuilder = FormBuilder::classMap($overview);

        if ($elementBuilder->has_element) {

            $data['elementsHeader'] = array_filter(FormBuilder::classMap($overview)->elementFields[Helper::config()->theme], function ($item) {
                return $item == 'Text' || $item == 'Upload';
            });
        }

        $data['title'] = "Manage {$overview} Section";

         return view('backend.frontend.indexOverview')->with($data);
    }
