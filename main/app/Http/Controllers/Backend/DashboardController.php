...
class DashboardController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        $data['metamask_ptop'] = Transaction::with('user')->whereIn('id',$metmaskUser)->sum('amount');
        $data['admin_ptop'] = Transaction::with('user')->whereNotIn('id',$metmaskUser)->where('type_two', '=',7)->where('type','+')->sum('amount');
        $data['ptop'] = Transaction::with('user')->where('type_two', '=', 6)->where('type', '+')->sum('amount');
