...
class ManageUserController extends Controller
{
    ...
    public function userDetails(Request $request)
    {
        ...
        $data['transactions'] = Transaction::with('user')
            ->when($request->trx, function ($item) use ($request) {
            ...
            ->when($request->date, function ($item) use ($request) {
                ...
                $start_date = \Carbon\Carbon::createFromFormat('m/d/Y', $start_date)->startOfDay();
                $end_date = \Carbon\Carbon::createFromFormat('m/d/Y', $end_date)->endOfDay();
...
//        })->where('user_id', $request->user)->latest()->with('user')->paginate(Helper::pagination());
