        ...
        <div class="col-xxl-12 col-lg-9 col-md-8">
            <div class="tab-content" id="v-pills-tabContent">
                <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
                    ...
                    @if (Builder::render('overview')['has_element'])
                        <div class="card">
                            <div class="card-header justify-content-between align-items-center">
                                <h5>{{ __(Str::ucfirst(str_replace('_', ' ', 'overview') . ' Elements')) }}</h5>
                                <a href="{{ route('admin.frontend.element', 'overview') }}"
                                   class="btn btn-sm btn-icon icon-left btn-primary add-page"> <i class="fa fa-plus"></i>
                                    {{ __('Add ' . str_replace('_', ' ', 'overview')) }}</a>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table class="table student-data-table m-t-20">
                                        ...
                                        @foreach ($elements as $el)
                                            <tr>
                                                @foreach ($valueFor as $key => $item)
                                                    <td>
                                                        @if ($item == 'Upload')
                                                            ...
                                                            <img src="{{ Config::getFile('overview', optional($el->content)->$key, true) }}"
                                                                 alt="">
                                                        @else
                                                ...
                                                @endforeach
                                                <td>
                                                    <a href="{{ route('admin.frontend.element.edit', ['name' => 'overview', 'element' => $el]) }}"
                                                       class="btn btn-sm py-1 btn-outline-primary"><i
                                                            class="fa fa-pen"></i></a>
                                                    <button class="btn btn-sm py-1 btn-outline-danger delete"
                                                            data-url="{{ route('admin.frontend.element.delete', ['overview', $el]) }}"><i
